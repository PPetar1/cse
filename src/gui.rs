...
//! map and lets the player click a hex to inspect its units. Read-only for
//! now — no orders issued from here yet (see docs/roadmap.md Phase 6).
//!
...
pub fn run(scenario_path: &str) -> Result<(), Error> {
    ...
    let game = Game::build(contents)?;

    eframe::run_native(
        "CSE",
        eframe::NativeOptions::default(),
        Box::new(|_cc| Ok(Box::new(GuiApp { game, selected_hex: None }))),
    )
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        if let Some((x, y)) = self.selected_hex {
            egui::Panel::right("inspector").min_size(240.0).show(ui, |ui| {
                render_inspector(ui, &self.game, x, y);
            });
        }
    ...
    fn render_map(&mut self, ui: &mut egui::Ui) {
        ...
        if response.clicked()
            && let Some(pos) = response.interact_pointer_pos() {
                self.selected_hex = view.hex_at(pos, &hex_set);
            }
    }
}

fn render_inspector(ui: &mut egui::Ui, game: &Game, x: u32, y: u32) {
