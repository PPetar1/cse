...
Commands:
  ...
  interdict <x> <y> <unit>            <unit> covers that hex this turn (up to 3 hexes per unit)
...
pub(crate) enum Command<'a> {
    ...
    Interdict { target: (u32, u32), unit: String },
    Interdiction,
...
impl<'a> Command<'a> {
    pub(crate) fn parse(input: &'a str) -> Result<Command<'a>, Error> {
        ...
        match keyword {
            ...
            "interdict" => {
                if args.len() < 3 {
                    return Err(Error::new("Need a hex and the covering unit's name for interdict."));
                }
                if let (Ok(x), Ok(y)) = (args[0].parse(), args[1].parse()) {
                    Ok(Command::Interdict { target: (x, y), unit: args[2..].join(" ") })
                } else {
                    Err(Error::new("Unable to parse arguments for interdict."))
                }
            }
            "interdiction" => Ok(Command::Interdiction),
            "simulate" => {
                if args.len() < 5 {
...
mod tests {
    ...
    #[test]
    fn parses_an_interdict_command_with_a_multi_word_unit_name() {
        let command = Command::parse("interdict 3 4 1st Fighter Regiment").unwrap();

        assert_eq!(
            command,
            Command::Interdict { target: (3, 4), unit: "1st Fighter Regiment".to_string() },
        );
    }

    #[test]
    fn rejects_interdict_with_missing_arguments() {
        let error = Command::parse("interdict 3 4").unwrap_err();

        assert!(error.error_message.contains("covering unit's name"));
    }

    #[test]
    fn parses_an_interdiction_command() {
        assert_eq!(Command::parse("interdiction").unwrap(), Command::Interdiction);
    }
