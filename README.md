...
| `end_turn` | pass control to the next player; once every player has moved, the turn counter and the in-game date advance. The faction coming on turn refills its movement points, its element morale drifts one step toward the faction default, and any of its units scheduled to arrive or leave this turn do so. If the scenario's `[victory_conditions] last_turn` has just been completed, prints the final score for every faction and the winner (or a draw) |
