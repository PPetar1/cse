...
| _(not a command — a separate launch mode)_ `cargo run -- --gui <scenario.scen>` | the real interface (Phase 6): opens a window showing the map, with a status header and a click-to-inspect side panel. Read-only for now — replaces `view`/`inspect`/`status`, not the rest of the terminal, which you still need for orders |
