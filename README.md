...
cargo run -- --gui scenarios/basic_scenario.scen   # the real interface (Phase 6, read-only so far)
```
...
| _(not a command — a separate launch mode)_ `cargo run -- --gui <scenario.scen>` | the real interface (Phase 6): opens a window showing the map, with a status header, an End Turn button, and a click-to-inspect side panel that offers Move/Attack buttons for a hex you control — click either, then a destination hex, to issue the order. `air_support`, `interdict`, and save/load still need the terminal |
