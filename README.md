...
| `move <x1> <y1> <x2> <y2> <unit_index>` | move the unit with the given index one hex, to an adjacent destination; costs movement points by terrain (refilled each turn from the TOE's `mp`), Water is impassable, and only the player on turn can move their units; stacked units are indexed in alphabetical order, matching the order `inspect` lists them |
